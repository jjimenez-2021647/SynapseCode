...
router.get('/', validateJWT, requireRole('USER_ROLE'), getRoomParticipations);

// Solo USER_ROLE puede listar integrantes de su sala
router.get('/room/:roomId', validateJWT, requireRole('USER_ROLE'), getRoomParticipationsByRoom);

router.get('/user/:userId', validateJWT, requireRole('USER_ROLE'), getRoomParticipationsByUser);
