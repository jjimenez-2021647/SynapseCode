...
const routes = (app) => {
    ...
    app.get(`${BASE_PATH}/Health`, (request, response) => {
        response.status(200).json({
            status: 'Healthy',
            service: 'SynapseCode-ServiceExecutionCode Running',
        })
    })
    app.use((req, res) => {
...
export const initServer = async () => {
    ...
    try {
        ...
        const swaggerDefinition = {
            openapi: '3.0.0',
            info: {
                title: 'SynapseCode-ServiceExecutionCode API',
                version: '1.0.0',
                description: 'Microservicio de Ejecución de Código - SynapseCode',
            },
            servers: [
                {
        ...
        routes(app);
        app.listen(PORT, () => {
            console.log(`SynapseCode-ServiceExecutionCode running on port ${PORT}`);
